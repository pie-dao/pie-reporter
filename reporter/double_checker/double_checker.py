...
def check_rewards(db, epoch):
    ...
    rewards_json = {
        to_checksum_address(key): int(val["amount"])
        for (key, val) in epoch["merkleTree"]["claims"].items()
    }
    ...
    common_addrs = [
        addr
        for addr in set([*rewards_db.keys(), *rewards_json.keys()])
        if addr in rewards_db.keys() and addr in rewards_json.keys()
    ]
            ...
            diff.append(
                {
                    "addr": addr,
                    "db": rewards_db[addr],
                    "json": rewards_json[addr],
                    "diff": rewards_db[addr] - rewards_json[addr],
                }
            )

    print(f"Differences found in: {len(diff)} accounts")
    total_diff = sum([d["diff"] for d in diff])
    total_db = sum([d["db"] for d in diff])
    total_json = sum([d["json"] for d in diff])

    print(f"{total_diff=} {total_db=} {total_json=}")
